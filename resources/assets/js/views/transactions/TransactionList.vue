<template>
    <div >
        <div class="btn-wrapper">
            <router-link to="/transactions/add" class="btn btn-primary btn-sm">Add Transaction</router-link>
        </div>
        <table class="table">
            <thead>
                <th>ID</th>
            ...
            <tbody>
                ...
                <template v-else>
                    <tr v-for="record in records" :key="record.id">
                        <td>{{ record.id }}</td> 
                        <td><router-link :to="`/members/${record.member_id}`">{{ record.member_name }}</router-link></td> 
                        <td>{{ record.amount }}</td>
                       
                    
                        <td>
                            <router-link :to="`/transactions/${record.id}`">View</router-link>
                        </td>
                    </tr>
                </template>
            </tbody>
    ...
    export default {
        ...
        methods: {
            fetchRecords(page_url){
                page_url = page_url || '/pipe/transactions'

                Transaction.all(page_url, records => { 
